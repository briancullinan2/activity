...
async function renderIndex() {
	...
	let historyStr = history
		...
		.map(item => {
			return `<div class="history-item"><span class="time">${item.start.getHours() % 12}:${item.start.getMinutes() < 10 ? '0' : ''}${item.start.getMinutes()} ${item.start.getHours() >= 12 ? 'pm' : 'am'}</span><span class="content">${item.content}</span></div>`
		}).join('\n')
