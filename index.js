	...
	*/
	...
	for(let i = 0; i < directories.length; i++) {
		...
		for(let j = 0; j < imageFiles.length; j++) {
			images += '<li class="' + directories[i].replace(/[^a-z0-9 ]/gi, '') + '" style="background-image:url(https://github.com/briancullinan2/clipart/blob/main/' + encodeURIComponent(directories[i]).replace(/\(/gi, '%28').replace(/\)/gi, '%29') + '/' + encodeURIComponent(imageFiles[j]) + '?raw=true)"></li>'
		}
	}
